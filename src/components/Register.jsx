    ...
    <>
      ...
      <Modal show={show} onHide={handleClose} contentClassName="bg-dark">
        <Modal.Header className="border-danger bg-dark">
          <Link className="mx-auto" to="/"><img className="bg-dark logo" src="/img/cine.jpg"width="100"height="100"/></Link>
        </Modal.Header>
      ...
      <Form.Group className="mb-3 bg-dark" controlId="formName">
        <Form.Label>Usuario {" "}
            {!validateName(name) && !firstName && (
              <span className="text-danger">Debe completar este campo</span>)}</Form.Label>
        <Form.Control value={name} onInput={(e) => setName(e.target.value)} onBlur={() => setFirstName(false)} className="p-3" type="text" placeholder="Name" />
      </Form.Group>
      ...
      <Form.Group className="mb-3" controlId="formPassword">
        <Form.Label>Contraseña{" "}
            {!validatePass(pass) && !firstPass && (
              <span className="text-danger">Debe completar este campo</span>)}</Form.Label>
        <Form.Control value={pass} onInput={(e) => setPass(e.target.value)} onBlur={()=> setFirstPass(false)} className="p-3" type="password" placeholder="Password" />
      </Form.Group>
